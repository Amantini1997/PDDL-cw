...
Domain and Problem files sourced from here: [pddl-instances/depot-strips-automatic](https://github.com/potassco/pddl-instances/tree/master/ipc-2002/domains/depots-strips-automatic)
...
### Additive
`CMD fd --overall-time-limit 30m --overall-memory-limit 2G --plan-file plan.txt ./domain.pddl ./problem.pddl --search "astar(add())"`
### Causal Graph
`CMD fd --overall-time-limit 30m --overall-memory-limit 2G --plan-file plan.txt ./domain.pddl ./problem.pddl --search "astar(cg())"`

## Testing Script (`experiment.py`)

Python 3 script for repeatedly running problems over a domain and multiple heuristics

Based off windows environment with the following files in the directory:
```
domain.pddl
student.Dockerfile.add
student.Dockerfile.csg
build.bat
run.bat
instance-1.pddl to instance-x.pddl
```
...
instance-`x`-`time_limit`-`memory_limit`-`<add-csg>`.txt

e.g. `instance-1-30m-2G-csg.txt`

### Content format

Just console output redirected to the file🤷‍♂️
